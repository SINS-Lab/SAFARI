...
def clearOutOfBounds(crystal, xmin, xmax, ymin, ymax, zmin, zmax):
    culled = []
    for i in range(len(crystal)):
        site = crystal[i]
        if site[0] < xmin or site[0] > xmax or site[1] < ymin or site[1] > ymax or site[2] < zmin or site[2] > zmax:
            continue
        culled.append(site)
    ...
    for x in range(-_n, _n):
        for y in range(-_n, _n):
            for z in range(-_n, _n):
                ...
                if (diff[2] + basis[maxZI][2]) <= zTop and \
                   (diff[2] + basis[minZI][2]) >= zBottom:
                    for i in range(len(basis)):
