...
C
C CHAIN VARIABLES
      REAL*4 XSTART,XSTEP,YSTART,YSTEP
      real*4 ztraj(narray)
      INTEGER NUMCHA
...
C    ION COORDINATES.
      ...
      P(2)=PY0
      P(3)=PZ0
C     EMO=(PX0**2 + PY0**2 + PZ0**2)/(2.*MION)
      EMO=DMAX1(DABS(PX0),DABS(PY0))
      EMO=DMAX1(EMO,DABS(PZ0))
C
C
C     WRITE(6,*) ' ****** SETTING PX = 0 *******'
C     P(1)=0.0D0
C
...
41       CONTINUE
         ...
         ZMIN=1.D+22
C
C               WILL LOOK FOR NN CLOSEST ATOMS, NN IS AT MOST 100.
...
C
      ...
      ENDIF
C
      CALL TRAJ(DELT,XDIST,YDIST,ZDIST,NPART1)
C
      NCALLS=NCALLS+1
C        NSTEPS=NSTEPS+STEPS
C
      IF(STUCK) GO TO 100
      IF(BURIED) GO TO 100
